...
\begin{document}
	...
	\section*{Lösen einer DGL 1. Ordnung}
	...
	\subsection*{Homogene Lösung: Trennung der Variablen}
	Finde ein $y$, welches die homogene Differentialgleichung (DGL) löst.
	\begin{mycapequ}[!ht]
		\vspace{-0.25cm}
		\begin{align*}
		i' + \frac{R}{L} \cdot i & = 0 \\
	...
	\begin{mycapequ}[!ht]
		\begin{mdframed}[leftmargin=0cm,
			...
			\begin{addmargin}[1em]{2em}% 1em left, 2em right
				...
				Durch \textit{Trennen der Variablen}. Die \textit{allgemeine Lösung} ist
	...
	\subsection*{Inhomogene Lösung: Variante der Konstanten}
	Eine \textit{inhomogene DGL 1. Ordnung} lässt sich durch die \textit{Variante der Konstanten} lösen.
	Dazu benutzt man die \textit{allgemeine Lösung} der \textit{homogenen DGL} und ersetzt die Integrationskonstante $C$ durch eine (noch unbekannte) Funktion $C\left(x\right)$ und berechnet mit dem Lösungsansatz $y_{h} \left( x \right) $ die Ableitung $y_{h}\mkern3mu' \left( x \right) $ unter Verwendung der \textit{Produkt- und Kettenregel}.
	\begin{mycapequ}[!ht]
		\vspace{-0.75cm}
		\begin{align*}
			% 1. Zeile
    ...
    Durch Kürzen bekommt man wieder eine einfachere Form.
	\begin{mycapequ}[!ht]
		\vspace{-0.50cm}
		\begin{align*}
			% 1. Zeile
	...
	Einsetzen des gelösten Integrals.
	\begin{mycapequ}[!ht]
		\vspace{-0.50cm}
		\begin{align*}
			% 1. Zeile
	...
	...die dann in den Lösungsansatz eingefügt wird und erhält nun die \textit{allgemeine Lösung
	der inhomogenen DGL}.
	\begin{mycapequ}[!ht]
		\vspace{-0.50cm}
		\begin{align*}
			...
			\cline{1-2}
			% 4. Zeile
			i \left( t \right) & = \dbox{$C\left(t\right)$} \cdot \mathrm{e}^{-\frac{R}{L} \, \cdot \, t} \\
			% 5. Zeile
